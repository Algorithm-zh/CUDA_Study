...
__global__ void reduceUnroll2(int * g_idata,int * g_odata,unsigned int n)
{
	...
	if (tid >= n) return;
	//convert global data pointer to the
	int *idata = g_idata + blockIdx.x*blockDim.x*2;//每个block处理2*blockDim个数据
	if(idx + blockDim.x < n)
	{
...
__global__ void reduceUnrollWarp8(int * g_idata,int * g_odata,unsigned int n)
{
	...
	if(idx+7 * blockDim.x<n)
	{
		...
		g_idata[idx]=a1+a2+a3+a4+a5+a6+a7+a8;//一个block的线程处理8个block的数据
	...
	//write result for this block to global mem
  //剩下的是一个线程束内的线程,不需要同步指令__syncthreads()，这样就少了5次同步指令，速度也会更快
	if(tid<32)//这里的判断会导致很多不会用到的线程仍在继续执行，不影响结果，因为统一线程束的线程是绝对并行的
	{
		volatile int *vmem = idata;//volatile是防止编译器自己优化数据传输打乱顺序
		vmem[tid]+=vmem[tid+32];//前32个tid得到64个数字的两两和，存到前32个数字里
		vmem[tid]+=vmem[tid+16];
		vmem[tid]+=vmem[tid+8];
		vmem[tid]+=vmem[tid+4];
		vmem[tid]+=vmem[tid+2];
		vmem[tid]+=vmem[tid+1];//最后的结果为tid[0]
	}

	if (tid == 0)
		g_odata[blockIdx.x] = idata[0];//每一个block把自己计算的最终结果写到g_odata
...
__global__ void reduceCompleteUnroll(int * g_idata,int * g_odata,unsigned int n)
{
	...
	__syncthreads();
  //使用模板的原因是让编译器在编译期就知道block的大小，自动去掉不必要的分支，直接生成对应大小的代码
	//in-place reduction in global memory
