...
__global__ void reduceNeighboredLess(int * g_idata,int *g_odata,unsigned int n)
{
    ...
    /*
     * tid和index对应关系：
     * tid是哪个线程，而index则是哪个数据
     *   tid  index
     *    1     0
     *    2     2
     *    3     4
     *    4     6
     * 也就是说，这让连续的线程映射到了对应的数据位置上（邻居相加）,上面那个函数则是单纯的线程id和数据下标对应了
     * 这样的话，可以保证前几个线程束可以拉满执行，后面的线程束全都不执行,避免了同一个线程内的线程束分化
     * (因为gpu里线程束是执行单位，每次有一个线程束会执行，如果分化了，正常执行有32个线程在并行，就会变成16个甚至更少的线程在并行)
     */
...
__global__ void reduceInterleaved(int * g_idata, int *g_odata, unsigned int n)
{
	...
	unsigned idx = blockIdx.x*blockDim.x + threadIdx.x;
	// convert global data pointer to the local point of this block
	int *idata = g_idata + blockIdx.x*blockDim.x;//每个块有自己需要处理的数据部分，所以需要转换为局部指针
	if (idx >= n)
		return;
	//in-place reduction in global memory
	for (int stride = blockDim.x/2; stride >0; stride >>=1)
	{
	  //该方法直接省掉了if,直接让线程处理对应的内存位置，但是内存对不相邻了，而是隔了一定的距离，线程内分化是最小的	
		if (tid < stride)
		{
			idata[tid] += idata[tid + stride];
		}
